...
function apply(ctx) {
  ...
  const compute = () => {
    ...
    const taskTop = tasks
      .filter((tk) => {
        const t = dayStartMs(dayKey(tk.time))
        return t >= weekStartMs && t < tomorrowStart
      })
      .sort((a, b) => b.total - a.total)
      .slice(0, 3)
      .map((tk) => ({
        title: tk.title, project: tk.project, time: tk.time, total: tk.total,
        cost: tk.cost, durationMs: tk.durationMs, in: tk.in, out: tk.out, cr: tk.cr,
      }))
