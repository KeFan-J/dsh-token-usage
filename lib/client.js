  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      function UsagePanel() {
        ...
        const taskTop = Array.isArray(stats.taskTop) ? stats.taskTop : []
        ...
        const taskTopRows = taskTop.length
          ? taskTop.map((tk, idx) => React.createElement('div', { className: 'tu-task-row', key: idx }, [
              ...
              React.createElement('div', { className: 'tu-rank-main' }, [
                ...
                React.createElement('div', { className: 'tu-model-sub' },
                  `项目 ${projName(tk.project)} · 时长 ${fmtDur(tk.durationMs)} · 费用 ≈ ${fmtMoney(tk.cost, sym)}`),
                React.createElement('div', { className: 'tu-model-sub' },
                  `输入 ${fmtInt(tk.in)} · 输出 ${fmtInt(tk.out)} · 命中 ${fmtInt(tk.cr)}`),
              ]),
              React.createElement('div', { className: 'tu-task-tokens' }, `${fmtInt(tk.total)} tokens`),
            ]))
          : React.createElement('div', { className: 'tu-hint' }, '本周暂无任务数据')
        ...
        return React.createElement('div', { className: 'tu-wrap' }, [
          ...
          React.createElement('div', { className: 'tu-models-card' }, [
            React.createElement('div', { className: 'tu-chart-head' },
              React.createElement('span', null, '任务 Token 消耗 Top 3（本周）')),
            taskTopRows,
          ]),
